...
import { LayoutDashboard, Package, Shapes, Users, ShoppingBag, RotateCcw, LogOut, Search, Plus, ArrowUpRight, ChevronRight, Check, Menu, X, Truck, ShieldCheck, Wallet, Store, Image, Tag } from 'lucide-react';
...
function Editor({ type, data, categories, busy, onSubmit }) {
  ...
  return <form className="editor" onSubmit={e => { e.preventDefault(); setError(''); const f = Object.fromEntries(new FormData(e.target)); try {
    if (type === 'Products') {
      ...
      onSubmit({ name: f.name, description: f.description, pricePaise: paise(f.retail), wholesalePaise: paise(f.wholesale), mrpPaise: f.mrp ? paise(f.mrp) : undefined, stock: Number(f.stock), categoryId: f.categoryId, refundWindowHours: Number(f.refundWindowHours), images: images.split('\n').map(x => x.trim()).filter(Boolean), colors: f.colors.split(',').map(x => x.trim()).filter(Boolean), sizes: f.sizes.split(',').map(x => x.trim()).filter(Boolean), colorImages: Object.keys(colorImageMap).length ? colorImageMap : undefined, attributes: cleanAttributes, deal: f.deal === 'on' });
    }
    else if (type === 'Categories') onSubmit({ name: f.name, icon: data?.icon || 'shopping_bag' });
    else if (type === 'Banners') onSubmit({ title: f.title, subtitle: f.subtitle || undefined, imageUrl: f.imageUrl || undefined, backgroundColor: f.backgroundColor || undefined, buttonText: f.buttonText || 'Shop Now', sortOrder: Number(f.sortOrder) || 0, active: f.active === 'on' });
    else if (type === 'Coupons') onSubmit({ code: f.code, description: f.description || '', discountType: f.discountType, value: f.discountType === 'PERCENT' ? Number(f.value) : paise(f.value), minOrderPaise: f.minOrderPaise ? paise(f.minOrderPaise) : 1, maxDiscountPaise: f.maxDiscountPaise ? paise(f.maxDiscountPaise) : undefined, usageLimit: f.usageLimit ? Number(f.usageLimit) : undefined, expiresAt: f.expiresAt ? new Date(f.expiresAt).toISOString() : undefined, active: f.active === 'on' });
    else onSubmit({
      name: f.name,
      ...
      aadharNumber: f.aadharNumber || undefined, panNumber: f.panNumber || undefined,
    ...
    {type === 'Coupons' && <>
      <Field label="Code" name="code" defaultValue={data?.code} placeholder="NTSA500" pattern="[A-Za-z0-9]+" minLength={3} maxLength={30} required disabled={!!data}/>
    ...
    {type === 'Vendors' && <>
      {!data && <div className="form-grid">
        ...
        <Field label="Password" name="password" type="password" placeholder="Set a password for this vendor" minLength={8} maxLength={100} required/>
      </div>}
...
function ResetPassword({ vendor, busy, onSubmit }) {
  ...
  return <form className="editor" onSubmit={e => { e.preventDefault(); setError(''); const f = Object.fromEntries(new FormData(e.target)); try { onSubmit(f.password); } catch (err) { setError(err.message); } }}>
    ...
    <Field label="New password" name="password" type="password" minLength={8} maxLength={100} required autoFocus/>
